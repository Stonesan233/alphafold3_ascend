...
def _resolve_af3_weight_file(model_path: pathlib.Path) -> pathlib.Path:
    """Return the AF3 weight file in ``model_path``, preferring .zst over .bin."""
...
def import_jax_weights_(model, model_path: pathlib.Path):
    ...
    for fourier_file in ('fourier_weight.npy', 'fourier_bias.npy'):
        if not (model_path / fourier_file).exists():
            raise FileNotFoundError(
                f'Required fourier embedding file not found: '
                f'{model_path / fourier_file}'
            )
