...
try:
    ...
    class _TritonStub:
        """No-op stand-in so module-level @triton.jit kernel definitions do
        not break import on machines without triton (e.g. Ascend NPU)."""
...
def gated_linear_unit(x, weight):
    implementation = fastnn_config.gated_linear_unit_implementation
    ...
    if implementation == "torch":
        ...
        raise ValueError(
            f"gated_linear_unit_implementation must be 'torch' or 'triton', "
            f"got {fastnn_config.gated_linear_unit_implementation}"
        )
    return out
